...
public class Smaz {
    ...
    public byte[] compress(String inString) {
        ...
        while ((inlen = charBuffer.remaining()) > 0) {
            int h1, h2, h3;
            charBuffer.mark();
            h1 = h2 = charBuffer.get() << 3; //get the current character and multiply the ASCII code with 2^3=8
            if (inlen > 1) h2 += charBuffer.get(); //if more than one character is remaining, read another one and concat it to h2
            if (inlen > 2) {
            ...
            for (; j > 0; j--) {
                CharBuffer slot;
                if (j == 1) {
                    slot = CharBuffer.wrap(CODEBOOK[h1 % CODEBOOK.length]);
                } else if (j == 2) {
                    slot = CharBuffer.wrap(CODEBOOK[h2 % CODEBOOK.length]);
                } else {
                    slot = CharBuffer.wrap(CODEBOOK[h3 % CODEBOOK.length]);
                }
    ...
    private void confirmOnlyAscii(String input) throws IllegalArgumentException{
        char[] chars = input.toCharArray();
        for (char c : chars)
            if (c > 127)
                throw new IllegalArgumentException("Only ASCII characters are allowed");
    }
